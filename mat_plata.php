...
$zapros= mysqli_query($link,"SELECT * FROM mat_plata");
...
<div class="way">
	<span>
		<a href="index.php"> / ГЛАВНАЯ </a>  <a href="kompl.php"> / КОМПЛЕКТУЮЩИЕ ДЛЯ ПК </a> / МАТЕРИНСКИЕ ПЛАТЫ
	</span>
</div>
<br>
<div id="out">
...
$chipset[$k]=$row['chipset'];
$chastota_spec[$k]=$row['chastota_spec'];
$slotov_pamyati[$k]=$row['slotov_pamyati'];
...
var id, name1, gnezdo_processora, chipset, chastota_spec,slotov_pamyati,form_factor, price, image, tovar={};
...
chipset=<?php echo json_encode($chipset);?>;
chastota_spec=<?php echo json_encode($chastota_spec);?>; 
slotov_pamyati=<?php echo json_encode($slotov_pamyati);?>; 
...
{ 
tovar={"name1" : name1[i], "gnezdo_processora" : gnezdo_processora[i], "chipset" : chipset[i], "chastota_spec": chastota_spec[i],"slotov_pamyati":slotov_pamyati[i],"form_factor":form_factor[i], "price": price[i], "image": image[i], "id" : id[i] }; 
arr.push(tovar); }}
...
out+='<h3> '+ "Чипсет: "+arr[key].chipset+'</h3>'+'<br>';
out+='<h3> '+ "Слотов памяти DDR4: "+arr[key].slotov_pamyati+'</h3>'+'<br>';
out+='<h3> '+ "Форм-фактор: "+arr[key].form_factor+'</h3>'+'<br>';
out+='<h3> '+"Частотная спецификация памяти: "+arr[key].chastota_spec+ '</h3>'+'<br>'+'</div>'; 
...
localStorage.setItem('mat_plata',JSON.stringify(basket));
