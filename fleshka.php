...
$zapros= mysqli_query($link,"SELECT * FROM flash_memory");
...
<div class="way">
	<span>
		<a href="index.php"> / ГЛАВНАЯ </a> / <a href="nositeli_inf.php">НОСИТЕЛИ ИНФОРМАЦИИ </a> / ФЛЕШКИ
	</span>
</div>
<div id="out">
...
var id, name1, interface, raziem, image, price,tovar={};
...
raziem=<?php echo json_encode($raziem);?>;
...
for (i=0; i < <?php echo $num_rows;?>; i++)
 {
tovar={"id":id[i],"name1":name1[i],"interface":interface[i], "raziem":raziem[i],"image":image[i], "price":price[i]}; 
arr.push(tovar); }}
...
out+='<h3> '+ "Разъем: "+arr[key].raziem+ '</h3>'+'</div>'; 
...
{
basket[articul_id]="flash_memory";
...
localStorage.setItem('flash_memory',JSON.stringify(basket));
